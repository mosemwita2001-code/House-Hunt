...
export default function PropertyDetails() {
  ...
  const [phone, setPhone] = useState(() => localStorage.getItem('viewer_phone') || '');
  ...
  const load = () => API.get(`/properties/${id}`, { params: phone ? { phone } : {} }).then(res => setProperty(res.data));
  useEffect(() => { load().catch(err => console.error('Error fetching property:', err)).finally(() => setLoading(false)); }, [id]);

  const payToView = async () => {
    if (!phone.trim() || !billing.firstName.trim() || !billing.lastName.trim() || !billing.email.trim()) return alert('Enter your name, email, and the M-Pesa number that will pay the KSh 40 view fee.');
    setPaymentLoading(true);
    try {
      localStorage.setItem('viewer_phone', phone.trim());
  ...
  const handleInquirySubmit = async e => {
    ...
    } catch (err) { alert(err.response?.data?.message || 'A successful KSh 40 view payment is required first.'); }
