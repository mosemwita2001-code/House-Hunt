...
export default function PropertyDetails() {
  ...
  return <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6">
    ...
    {!full && <div className="mt-8 bg-amber-50 border border-amber-200 rounded-2xl p-5 max-w-2xl"><div className="flex items-center gap-2 font-semibold"><Lock size={17} /> Full location, description and contact details are locked</div><p className="text-sm text-slate-600 mt-2">Pay KSh 40 through PesaPal to view the full listing.</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4"><input className="p-3 rounded-xl border" placeholder="First name" value={billing.firstName} onChange={e => setBilling({ ...billing, firstName: e.target.value })} /><input className="p-3 rounded-xl border" placeholder="Last name" value={billing.lastName} onChange={e => setBilling({ ...billing, lastName: e.target.value })} /><input className="p-3 rounded-xl border" type="email" placeholder="Email address" value={billing.email} onChange={e => setBilling({ ...billing, email: e.target.value })} /><input className="p-3 rounded-xl border" placeholder="M-Pesa phone e.g. 0712345678" value={phone} onChange={e => setPhone(e.target.value)} /></div>{paymentError && <p className="mt-3 text-sm text-red-600" role="alert">{paymentError}</p>}<button onClick={payToView} disabled={paymentLoading} className="bg-slate-900 text-white px-4 py-3 rounded-xl mt-2">{paymentLoading ? 'Starting...' : 'Pay KSh 40'}</button></div>}
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
      ...
      {full && <form onSubmit={handleInquirySubmit} noValidate className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-4"><h3 className="font-bold text-lg">Send an Inquiry</h3><input type="text" placeholder="Your Name" className="w-full p-3 rounded-xl border" value={inquiry.name} onChange={e => setInquiry({ ...inquiry, name: e.target.value })} /><input type="email" placeholder="Your Email" className="w-full p-3 rounded-xl border" value={inquiry.email} onChange={e => setInquiry({ ...inquiry, email: e.target.value })} /><input type="tel" placeholder="M-Pesa number e.g. 0712345678" className="w-full p-3 rounded-xl border" value={phone} onChange={e => setPhone(e.target.value)} /><textarea placeholder="Message" className="w-full p-3 rounded-xl border h-24" value={inquiry.message} onChange={e => setInquiry({ ...inquiry, message: e.target.value })} />{inquiryError && <p className="text-sm text-red-600" role="alert">{inquiryError}</p>}<button disabled={submitting} type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium flex items-center justify-center gap-2">{submitting ? 'Sending...' : <><Send size={16} /> Send Inquiry</>}</button></form>}
      </div>
    </div>
  </div>;
}
